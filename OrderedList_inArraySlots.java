...
public class OrderedList_inArraySlots
    ...
    public int indexOf( Integer findMe) {
        return indexOfRecursive(0, list_iAS.size()-1, findMe);
        //return indexOfWhile(findMe);
    }

    private int indexOfRecursive(int start, int end, Integer value) {
        if (start == end) {
            return value == get(start) ? start : -1;
        }
        int mid = (end + start) / 2;
        if (value <= get(mid)) {
            return indexOfRecursive(start, mid, value);
        } else {
            return indexOfRecursive(mid+1, end, value);
        }
    }

    private int indexOfWhile(Integer value) {
        int start = 0;
        int end = list_iAS.size()-1;
        while (start!=end) {
            int mid = (end + start) / 2;
            if (value <= get(mid)) {
                end = mid;
